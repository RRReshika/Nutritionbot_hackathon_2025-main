...
interface LayeredChatProps {
  ...
  onGreeting?: () => void;
  onCaloriesDetected?: (calories: number | null, foodName: string, answerType?: 'myth' | 'fact' | 'general') => void;
...
export default function LayeredChat({ onTypingChange, onGreeting, onCaloriesDetected, onAnswerTypeChange, externalMessage, onExternalMessageProcessed, onMyTakeChange }: LayeredChatProps) {
  ...
  const handleButtonClick = async (buttonValue: string, originalQuery: string, messageId: string) => {
    ...
    try {
      ...
      // Notify parent of answer type and myTake
      ...
      // Trigger hologram for myth/fact responses
      if ((answerType === 'myth' || answerType === 'fact') && onCaloriesDetected) {
        onCaloriesDetected(1, originalQuery.substring(0, 50), answerType);
      }
  ...
  const handleSend = async (text: string) => {
    ...
    try {
      ...
      console.log(`📊 Backend response - Type: ${data.type}, Detected as: ${answerType}`);
      
      // Notify parent of answer type
      if (onAnswerTypeChange) {
        onAnswerTypeChange(answerType);
        console.log(`🎨 Setting answer type to: ${answerType}`);
      }
      ...
      if (calorieMatch && onCaloriesDetected) {
        ...
        onCaloriesDetected(calories, foodName, answerType);
      } else if ((answerType === 'myth' || answerType === 'fact') && onCaloriesDetected) {
        // For myth/fact without calories, use 1 to trigger hologram with just status
        console.log(`🔍 Detected ${answerType} without calories for: ${text}`);
        onCaloriesDetected(1, text.substring(0, 50), answerType); // Pass answerType directly!
      } else if (onCaloriesDetected) {
        onCaloriesDetected(null, '', 'general');
      }
