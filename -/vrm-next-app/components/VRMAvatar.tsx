...
export default function VRMAvatar({ isTyping = false, isWaving = false, calories = null, foodName = '', answerType = 'general', myTake = null }: VRMAvatarProps) {
  ...
  const createIdleHologram = (vrm: any): THREE.Group => {
    ...
    // Blue color for idle state
    const primaryColor = new THREE.Color(0x0099ff);  // Blue
    const secondaryColor = new THREE.Color(0x00ccff); // Light blue
    ...
    // MULTIPLE TRANSPARENT GLOWING RINGS
    const ringLayers = [
    ...
    // Empty center - no text, just the hologram effect
    ...
    console.log('✓ Idle blue hologram created (no number)');
  ...
  const createCalorieHologram = (vrm: any, calories: number, foodName: string, isMythOrFact: 'myth' | 'fact' | 'general' = 'general'): THREE.Group => {
    console.log(`🎨 createCalorieHologram called with isMythOrFact: "${isMythOrFact}" (type: ${typeof isMythOrFact})`);
  ...
  useEffect(() => {
    console.log(`🔮 VRMAvatar received - calories: ${calories}, answerType: ${answerType}, foodName: ${foodName}`);
    ...
    } else if (vrmRef.current && calories === null) {
      // Show idle blue hologram when no calories
      if (calorieHologramRef.current && hologramAnchorRef.current) {
        const leftHandBone = vrmRef.current.humanoid.getBoneNode('leftHand');
        if (leftHandBone && hologramAnchorRef.current.parent === leftHandBone) {
          leftHandBone.remove(hologramAnchorRef.current);
          console.log('✓ Removed calorie hologram');
        }
      }
      
      const idleHologram = createIdleHologram(vrmRef.current);
      calorieHologramRef.current = idleHologram;
      console.log('✓ Idle blue hologram displayed');
    }
      ...
      (gltf: any) => {
        try {
          ...
          if (vrm) {
            ...
            // Reset VRM to default pose (arms down) before applying custom rotations
            if (vrm.humanoid) {
            ...
            // Create idle blue hologram on load (no number)
            setTimeout(() => {
              if (vrmRef.current) {
                const idleHologram = createIdleHologram(vrmRef.current);
                calorieHologramRef.current = idleHologram;
                console.log('✨ Idle blue hologram created on load!');
              }
            }, 500);
          } else {
